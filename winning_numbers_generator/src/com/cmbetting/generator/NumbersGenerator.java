...
//        playPick3();
    ...
    private static void playLotto() {
        List<Integer> toPlayNumbers = getNumbers(LOTTO_NUM_BALLS, LOTTO_MAX, false);
        print("lotto", toPlayNumbers);
    }

    private static void playPowerball() {
        List<Integer> toPlayNumbers = getNumbers(POWERBALL_NUM_BALLS, POWERBALL_OTHERS_MAX, false);
        print("powerball", toPlayNumbers);

        List<Integer> theRealPB = getNumbers(1, POWERBALL_MAX, false);
        print("The real powerball", theRealPB);

    }

    private static void playPick3() {
        List<Integer> toPlayNumbers = getNumbers(PICK3_NUM_BALLS, PICK3_MAX, true);
        print("pick3", toPlayNumbers);
    }

    private static List<Integer> getNumbers(int numberOfBalls, int maxBall, boolean allowDuplicates) {
